...
// initializes element of polynomial
...
//adds the term of polynomial to the array
...
struct poly polyadd(struct poly p1, struct poly p2) {
    ...
    for(i = 0,j=0;i<=c;p3.noofterms++) {
        if(p1.t[i].coeff == 0 && p3.t[j].coeff == 0) {
            break;
        }
...
//multiply two polynomial p1 and p2
struct poly polynomial(struct poly p1, struct poly p2){
    int coeff,exp;
    struct poly temp,p3;
    initpoly(&temp);
    initpoly(&p3);
